...
def check_login(username, password, json_file_path="data.json"):
    try:
        ...
        for user in data["users"]:
            if user["email"] == username and user["password"] == password:  # Change "username" to "email"
                session_state["logged_in"] = True
                session_state["user_info"] = user
            ...
            def process_csv_with_delay(sampled_data, model, st):
                while st.session_state.is_running:
                    ...
                    if prediction[0] != 'BENIGN':
                        prediction_placeholder.markdown(f"<p style='color:red; font-size:20px;'>Time : {timestamp} - Danger !!! You are Under Attack ----> Attack Type: {prediction[0]} </p>", unsafe_allow_html=True)
                    else:
